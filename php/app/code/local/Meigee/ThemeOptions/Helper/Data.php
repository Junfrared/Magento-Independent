 ...
 public function getThemeOptions ($themeOption) {
 	switch ($themeOption) {
	    ...
	    case 'productpage_pagelayout':
 		...
 		break;
		case 'moreview_button':
 			return Mage::getStoreConfig('meigee_productpage/productpage/moreviewsbutton');
 		break;
 ...
 public function prevnext ($product) {
 	if ($this->getThemeOptions('productpage_prevnext') == 'prevnext'):
		...
		*/
	    ...
	    $prev_next_divider = '';
	    if ($url <> $next_url): 
		$html = '<a class="product-prev" href="' . $next_url . '">' . $this->__('Previous') . '</a>';
		$prev_next_divider = '<span class="prev-next-divider">&nbsp;</span>';
	    endif;
	    if ($url <> $prev_url):
	        $html .= '<a class="product-next" href="' . $prev_url . '">'.$prev_next_divider.$this->__('Next') . '</a>';
	    endif;
